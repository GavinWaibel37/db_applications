...
SELECT player_id, pts, gp, pts / gp AS ppg
FROM player_season_stats
WHERE gp > 0
ORDER BY ppg DESC;

-- 5. Same as query 4, but round points per game to one decimal place.
SELECT player_id, pts, gp, ROUND(pts / gp) AS ppg
FROM player_season_stats
WHERE gp > 0
ORDER BY ppg DESC;
...
SELECT team_id, SUBSTR(season, 1, 4)
...
--    The numbers got rounded to whole numbers
...
FROM player_season_stats
WHERE gp = 0;
-- There are no players with gp = 0, but you put that parameter in either 
-- way to make sure the program runs without issues

-- =====================================================================
-- VOCABULARY — your words, not the reference sheet's
-- =====================================================================

-- Calculated column: A created column that has calculated data
-- Concatenation (||): Combining two things of data